...
# アクション可能であるプレイヤーに対する処理
    execute if entity @e[type=minecraft:chest_minecart, tag=InvGui.RegisteredEntity, limit=1] as @a at @s if entity @e[type=minecraft:chest_minecart, tag=InvGui.RegisteredEntity, distance=..10.0, limit=1] run function inventory_gui:core/action/check_action
