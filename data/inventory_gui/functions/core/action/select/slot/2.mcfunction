...
# 別のアイテムが入っていれば返還
    execute if data storage inventory_gui:temp CurrentInventory[{Slot:2b}] run data modify storage inventory_gui:temp ShulkerItems set from storage inventory_gui:temp CurrentInventory[{Slot:2b}]
